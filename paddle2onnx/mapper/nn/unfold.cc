...
void UnfoldMapper::Opset11() {
  ...
  Assert(paddings_[0] == paddings_[2] && paddings_[1] == paddings_[3],
  ...
  padded_input =
      _get_im2col_padded_input(input_info[0].name, padding_h, padding_w);
  auto gather_node1 =
  ...
  blocks_d =
      helper_
          ->MakeNode(
              ...
              {intput_d,
               helper_->Constant(
                   {}, ONNX_NAMESPACE::TensorProto::INT64, padding_d * 2)})
          ->output(0);
  blocks_d =
      helper_
          ->MakeNode("Sub",
...
std::string UnfoldMapper::_get_im2col_padded_input(std::string &input_name,
                                                   int64_t padding_h,
                                                   int64_t padding_w) {
  std::vector<int64_t> pad_constant{
      0, 0, padding_h, padding_w, 0, 0, padding_h, padding_w};
  std::string pad =
