...
std::string OnnxHelper::Constant(const std::string &output,
  ...
  if (dtype == ONNX_NAMESPACE::TensorProto::FLOAT) {
    ...
    Assert(false,
           "Only support data type of "
           "BOOL/FLOAT/DOUBLE/INT32/INT64/INT8 in Constant function.");
  }
  nodes.push_back(node);
  return output;
}
...
std::string OnnxHelper::ConstOfShape(const std::string &input,
  ...
  if (dtype == ONNX_NAMESPACE::TensorProto::FLOAT) {
    ...
    Assert(false,
           "Only support data type of FLOAT/DOUBLE/INT64/INT32 in "
           "ConstOfShape function.");
  }
  return output;
}
...
std::string OnnxHelper::Assign(
  ...
  Assert(numel == value.size(),
         "Numel of value not satisfy the input shape "
         "while creating contant tensor.");
  for (size_t i = 0; i < shape.size(); ++i) {
    tensor->add_dims(shape[i]);
  }
  tensor->set_data_type(dtype);
  if (dtype == ONNX_NAMESPACE::TensorProto::FLOAT) {
    ...
    Assert(false,
           "Only support data type of FLOAT/DOUBLE/INT32/INT64 in "
           "Constant function.");
  }
  nodes.push_back(node);
  return output;
