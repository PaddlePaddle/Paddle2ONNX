...
void IndexPutMapper::Opset11() {
  ...
  // - indices: a vector of index tensors (one per dimension, can be boolean masks)
  ...
  if (is_boolean_mask) {
    ...
    std::string value_broadcast = helper_->MakeNode(
        "Expand", {value_name, x_shape_node->output(0)})->output(0);
    
    if (accumulate_) {
      // When accumulate is true: x[mask] += value
      // out = Where(mask, x + value_broadcast, x)
      std::string add_result = helper_->MakeNode(
          "Add", {x_info[0].name, value_broadcast})->output(0);
      helper_->MakeNode(
          "Where", {mask, add_result, x_info[0].name}, {output_info[0].name});
    } else {
      // out = Where(mask, value_broadcast, x)
      helper_->MakeNode(
          "Where", {mask, value_broadcast, x_info[0].name}, {output_info[0].name});
    }
  } else {
    ...
    for (size_t i = 0; i < indices_info.size(); ++i) {
      ...
      std::string axes_node = helper_->Constant(ONNX_NAMESPACE::TensorProto::INT64, 
                                                 std::vector<int64_t>{-1});
      auto unsqueeze_node = helper_->MakeNode("Unsqueeze", {idx_name, axes_node});
    ...
    auto indices_shape_node = helper_->MakeNode("Shape", {indices_info[0].name});
    ...
    auto data_shape_suffix = helper_->MakeNode(
        "Slice", {data_shape_node->output(0), start_const, end_const, axes_const})->output(0);
    ...
    value_name = helper_->MakeNode(
        "Expand", {value_name, target_shape_node->output(0)})->output(0);
    ...
    if (accumulate_) {
      ...
      helper_->MakeNode(
          "Add", {x_info[0].name, scatter_node->output(0)}, {output_info[0].name});
    } else {
      helper_->MakeNode(
          "ScatterND", {x_info[0].name, indices_concat, value_name},
          {output_info[0].name});
    }
  }
}
