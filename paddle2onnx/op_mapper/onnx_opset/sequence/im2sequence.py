...
class BoxCoder():
    ...
    def opset_9(cls, node, **kw):
        ...
        for i in range(out_h):
            for j in range(out_w):
                ...
                slice_block_node = helper.make_node(
                    'Slice',
                    inputs=[node.input('X')[0], starts_name, ends_name],
        ...
        concat_block_name = "im2sequence.concat_block.{}".format(
            self.im2seq_counter)
        ...
        concat_block_node = helper.make_node(
            "Concat", inputs=slice_blocks, outputs=node.output('Out'), axis=0)
