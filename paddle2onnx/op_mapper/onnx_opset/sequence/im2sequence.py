    ...
    def opset_9(cls, graph, node, **kw):
        ...
        for i in range(out_h):
            for j in range(out_w):
                ...
                slice_block_node = graph.make_onnx_node(
                    'Slice',
                    inputs=[node.input('X', 0), starts_node, ends_node])
                flatten_block_node = graph.make_onnx_node(
                    "Flatten", inputs=[slice_block_node], axis=0)
                nodes.extend([slice_block_node, flatten_block_node])
        #    concat_block_node = graph.make_onnx_node("Concat", inputs=slice_blocks, outputs=[concat_block_name], axis=0)
        concat_block_node = graph.make_onnx_node(
            "Concat",
