...
class MultiClassNMS():
    ...
    def opset_10(cls, graph, node, **kw):
        ...
        node_score_threshold = graph.make_onnx_node(
            'Constant',
            inputs=[],
            dtype=dtypes.ONNX.FLOAT,
            value=[float(node.attr('score_threshold'))])

        node_iou_threshold = graph.make_onnx_node(
            'Constant',
            inputs=[],
            dtype=dtypes.ONNX.FLOAT,
            value=[float(node.attr('nms_threshold'))])

        node_keep_top_k = graph.make_onnx_node(
            'Constant',
            inputs=[],
            dtype=dtypes.ONNX.INT64,
            value=[np.int64(node.attr('keep_top_k'))])

        node_keep_top_k_2D = graph.make_onnx_node(
            'Constant',
            inputs=[],
            dtype=dtypes.ONNX.INT64,
            dims=[1, 1],
        ...
        node_select_nms= graph.make_onnx_node(
            'NonMaxSuppression',
        ...
        node_gather_1 = graph.make_onnx_node(
            'Gather',
            inputs=[node_select_nms, result_name + "@const_1"],
            axis=1)

        node_squeeze_gather_1 = graph.make_onnx_node(
            'Squeeze', inputs=[node_gather_1], axes=[1])

        node_gather_2 = graph.make_onnx_node(
            'Gather',
        ...
        node_gather_topk_scores = graph.make_onnx_node(
            'Gather',
            inputs=[
        ...
        node_gather_topk_class = graph.make_onnx_node(
            'Gather',
            inputs=[
        ...
        node_gather_topk_boxes_id = graph.make_onnx_node(
            'Gather',
            inputs=[
        ...
        node_squeeze_topk_boxes_id = graph.make_onnx_node(
            'Squeeze', inputs=[node_gather_topk_boxes_id], axes=[0, 2])

        node_gather_select_boxes = graph.make_onnx_node(
            'Gather',
        ...
        node_cast_topk_class = graph.make_onnx_node(
            'Cast', inputs=[node_gather_topk_class], to=1)

        node_unsqueeze_topk_scores = graph.make_onnx_node(
        ...
        node_sort_by_socre_results = graph.make_onnx_node(
            'Concat', inputs=inputs_concat_final_results, axis=2)

        # select topk classes indices
        node_squeeze_cast_topk_class = graph.make_onnx_node(
