...
class YOLOBox():
    ...
    def front(cls, graph, node, **kw):
        ...
        node_x_shape = graph.make_onnx_node(
            'Constant', attrs={'dtype': dtypes.ONNX.INT64,
                               'value': x_shape})

        node_x_reshape = graph.make_onnx_node(
            'Reshape', inputs=[node.input('X')[0], node_x_shape])
        node_x_transpose = graph.make_onnx_node(
        ...
        node_range_x = graph.make_onnx_node(
            'Constant', attrs={
                'dtype': dtypes.ONNX.FLOAT,
                'value': range_x,
            })

        node_range_y = graph.make_onnx_node(
            'Constant',
            inputs=[],
            attrs={
                'dtype': dtypes.ONNX.FLOAT,
        ...
        node_range_x_new_shape = graph.make_onnx_node(
            'Constant',
            inputs=[],
            dtype=dtypes.ONNX.INT64,
            value=range_x_new_shape)
        node_range_y_new_shape = graph.make_onnx_node(
            'Constant',
            inputs=[],
            dtype=dtypes.ONNX.INT64,
            value=range_y_new_shape)

        node_range_x_reshape = graph.make_onnx_node(
            'Reshape', inputs=[node_range_x, node_range_x_new_shape])
        node_range_y_reshape = graph.make_onnx_node(
            'Reshape', inputs=[node_range_y, node_range_y_new_shape])

        node_grid_x = graph.make_onnx_node(
            "Tile", inputs=[node_range_x_reshape, node_range_y_new_shape])

        node_grid_y = graph.make_onnx_node(
        ...
        node_split_input = graph.make_onnx_node(
            "Split",
        ...
        node_box_x_sigmoid = graph.make_onnx_node(
            "Sigmoid", inputs=[node_box_x])

        node_box_y_sigmoid = graph.make_onnx_node(
            "Sigmoid", inputs=[node_box_y])

        node_box_x_squeeze = graph.make_onnx_node(
            'Squeeze', inputs=[node_box_x_sigmoid], axes=[4])

        node_box_y_squeeze = graph.make_onnx_node(
            'Squeeze', inputs=[node_box_y_sigmoid], axes=[4])

        node_box_x_add_grid = graph.make_onnx_node(
            "Add", inputs=[node_grid_x, node_box_x_squeeze])

        node_box_y_add_grid = graph.make_onnx_node(
            "Add", inputs=[node_grid_y, node_box_y_squeeze])

        node_input_h = graph.make_onnx_node(
            'Constant',
            inputs=[],
            dtype=dtypes.ONNX.FLOAT,
            value=[input_height])

        node_input_w = graph.make_onnx_node(
            'Constant', inputs=[], dtype=dtypes.ONNX.FLOAT,
            value=[input_width])

        node_box_x_encode = graph.make_onnx_node(
            'Div', inputs=[node_box_x_add_grid, node_input_w])

        node_box_y_encode = graph.make_onnx_node(
            'Div', inputs=[node_box_y_add_grid, node_input_h])

        node_anchor_tensor = graph.make_onnx_node(
            "Constant", inputs=[], dtype=dtypes.ONNX.FLOAT, value=anchors)

        anchor_shape = [int(num_anchors), 2]
        node_anchor_shape = graph.make_onnx_node(
            "Constant", inputs=[], dtype=dtypes.ONNX.INT64, value=anchor_shape)

        node_anchor_tensor_reshape = graph.make_onnx_node(
            "Reshape", inputs=[node_anchor_tensor, node_anchor_shape])

        node_input_size = graph.make_onnx_node(
            "Constant", inputs=[], dtype=dtypes.ONNX.FLOAT, value=[input_size])

        node_anchors_div_input_size = graph.make_onnx_node(
            "Div", inputs=[node_anchor_tensor_reshape, node_input_size])

        node_anchor_w = model_name + "@anchor_w"
        node_anchor_h = model_name + "@anchor_h"

        node_anchor_split = graph.make_onnx_node(
            'Split',
        ...
        node_new_anchor_shape = graph.make_onnx_node(
            'Constant',
            inputs=[],
            dtype=dtypes.ONNX.INT64,
            value=new_anchor_shape)

        node_anchor_w_reshape = graph.make_onnx_node(
            'Reshape', inputs=[node_anchor_w, node_new_anchor_shape])

        node_anchor_h_reshape = graph.make_onnx_node(
            'Reshape', inputs=[node_anchor_h, node_new_anchor_shape])

        node_box_w_squeeze = graph.make_onnx_node(
            'Squeeze', inputs=[node_box_w], axes=[4])

        node_box_h_squeeze = graph.make_onnx_node(
            'Squeeze', inputs=[node_box_h], axes=[4])

        node_box_w_exp = graph.make_onnx_node(
            "Exp", inputs=[node_box_w_squeeze])
        node_box_h_exp = graph.make_onnx_node(
            "Exp", inputs=[node_box_h_squeeze])

        node_box_w_encode = graph.make_onnx_node(
            'Mul', inputs=[node_box_w_exp, node_anchor_w_reshape])

        node_box_h_encode = graph.make_onnx_node(
            'Mul', inputs=[node_box_h_exp, node_anchor_h_reshape])

        node_conf_sigmoid = graph.make_onnx_node('Sigmoid', inputs=[node_conf])

        node_conf_thresh = graph.make_onnx_node(
            'Constant',
            inputs=[],
            dtype=dtypes.ONNX.FLOAT,
        ...
        node_conf_shape = graph.make_onnx_node(
            'Constant', inputs=[], dtype=dtypes.ONNX.INT64, value=conf_shape)

        node_conf_thresh_reshape = graph.make_onnx_node(
            'Reshape', inputs=[node_conf_thresh, node_conf_shape])

        node_conf_sub = graph.make_onnx_node(
            'Sub', inputs=[node_conf_sigmoid, node_conf_thresh_reshape])

        node_conf_clip = graph.make_onnx_node('Clip', inputs=[node_conf_sub])

        zeros = [0]
        node_zeros = graph.make_onnx_node(
            'Constant', inputs=[], dtype=dtypes.ONNX.FLOAT, value=zeros)

        node_conf_clip_bool = graph.make_onnx_node(
            'Greater', inputs=[node_conf_clip, node_zeros])

        node_conf_clip_cast = graph.make_onnx_node(
            'Cast', inputs=[node_conf_clip_bool], to=1)

        node_conf_set_zero = graph.make_onnx_node(
            'Mul', inputs=[node_conf_sigmoid, node_conf_clip_cast])

        node_prob_sigmoid = graph.make_onnx_node('Sigmoid', inputs=[node_prob])

        new_shape = [1, int(num_anchors), input_height, input_width, 1]
        node_new_shape = graph.make_onnx_node(
            'Constant',
            inputs=[],
            dtype=dtypes.ONNX.INT64,
            dims=[len(new_shape)],
        ...
        node_pred_box_split = graph.make_onnx_node(
            'Split',
    ...
    def opset_9(cls, graph, node, **kw):
        ...
        node_score_shape = graph.make_onnx_node(
            "Constant",
            inputs=[],
            dtype=dtypes.ONNX.INT64,
            value=cls.score_shape)

        node_score_new_shape = graph.make_onnx_node(
            'Reshape',
    ...
    def opset_11(cls, graph, node, **kw):
        cls.front(graph, node, **kw)
        min_const = graph.make_onnx_node(
            'Constant', inputs=[], dtype=dtypes.ONNX.FLOAT, value=0.0)

        max_const = graph.make_onnx_node(
