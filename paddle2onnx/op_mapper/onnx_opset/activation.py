...
class Swish():
    @classmethod
    def opset_9(cls, graph, node, **kw):
        beta_node = graph.make_onnx_node(
            'Constant',
            attrs={'dtype': dtypes.ONNX.FLOAT,
                   'value': [node.attr('beta')]})
        beta_x_node = graph.make_onnx_node(
            'Mul', inputs=[node.input('X')[0], beta_node])
        sigmoid_node = graph.make_onnx_node('Sigmoid', inputs=[beta_x_node])
        graph.make_onnx_node(
            'Mul',
...
class HardSwish():
    ...
    def opset_9(cls, graph, node, **kw):
        ...
        offset_node = graph.make_onnx_node(
            'Constant',
            attrs={'dtype': dtypes.ONNX.FLOAT,
                   'value': node.attr('offset')})

        node0 = graph.make_onnx_node(
            'Add', inputs=[node.input('X')[0], offset_node])
        min_value = 0.0
        max_value = node.attr('threshold')
        node1 = graph.make_onnx_node(
            'Clip', inputs=[node0], max=max_value, min=min_value)
        node2 = graph.make_onnx_node('Mul', inputs=[node.input('X')[0], node1])
        node3 = graph.make_onnx_node(
