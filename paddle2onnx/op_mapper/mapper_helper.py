...
def get_another_node_by_input(graph, name, copy_node=False):
    check_op_list = [
    ...
    result_layer_name = []
    for layer_name, node in graph.ctx.node_map.items():
        inputs = node.inputs
    ...
    if copy_node:
        for i in range(len(result_layer_name)):
            node = copy.copy(graph.ctx.node_map[result_layer_name[i]])
            node_list.append(node)
    else:
        for i in range(len(result_layer_name)):
            node = graph.ctx.node_map[result_layer_name[i]]
            node_list.append(node)
    return node_list
