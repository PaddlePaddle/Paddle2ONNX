...
namespace PaddleOCR {
    ...
    PostProcessor::BoxesFromBitmap(
        ...
        for (int _i = 0; _i < num_contours; _i++) {
            ...
            for (int num_pt = 0; num_pt < 4; num_pt++) {
                std::vector<int> a{int(clampf(roundf(cliparray[num_pt][0] / float(width) * float(dest_width)), 0,
                                              float(dest_width))),
    ...
    std::vector <std::vector<std::vector < int>>>
    PostProcessor::FilterTagDetRes(std::vector<std::vector < std::vector < int>>
    > boxes,
    float ratio_h,
    float ratio_w, cv::Mat
    srcimg) {
