...
class SymbolicShapeInference:
    ...
    def _infer_symbolic_compute_ops(self, node):
        funcs = {
            "Add": lambda i: i[0] + i[1],
            "Div": lambda i: i[0] // i[1],  # integer div in sympy
            "Equal": lambda i: i[0] == i[1],
            "Floor": lambda i: sympy.floor(i[0]),
            "Max": lambda i: (
                i[1]
                if is_literal(i[0]) and int(i[0]) < -self.int_max_
                else (
                    i[0]
                    if is_literal(i[1]) and int(i[1]) < -self.int_max_
                    else sympy.Max(i[0], i[1])
                )
            ),
            "Min": lambda i: (
                i[1]
                if is_literal(i[0]) and int(i[0]) > self.int_max_
                    ...
                    i[0]
                    if is_literal(i[1]) and int(i[1]) > self.int_max_
                    else sympy.Min(i[0], i[1])
                )
            ),
            "Mul": lambda i: i[0] * i[1],
            "Sub": lambda i: i[0] - i[1],
            "Where": lambda i: i[1] if i[0] else i[2],
            "Neg": lambda i: -i[0],
        }
