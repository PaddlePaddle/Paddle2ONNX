...
void TritonInferenceEngine::Init(const std::string &url, bool verbose) {
    FAIL_IF_ERR(nic::InferenceServerHttpClient::Create(&client_, url, verbose),
                "error: unable to create client for inference.")}

nic::Error TritonInferenceEngine::GetModelMetaData(
    const std::string &model_name, const std::string &model_version,
    const nic::Headers &headers, rapidjson::Document *model_metadata) {
  std::string model_metadata_str;
  FAIL_IF_ERR(client_->ModelMetadata(&model_metadata_str, model_name,
                                     model_version, headers),
...
void TritonInferenceEngine::ParseConfigs(const TritonInferenceConfigs &configs,
...
void TritonInferenceEngine::Infer(const TritonInferenceConfigs &configs,
                                  const std::vector<DataBlob> &input_blobs,
                                  std::vector<DataBlob> *output_blobs,
                                  const nic::Headers &headers,
                                  const nic::Parameters &query_params) {
  nic::InferOptions options(configs.model_name_);
  ParseConfigs(configs, &options);

  rapidjson::Document model_metadata;
  GetModelMetaData(options.model_name_, options.model_version_, headers,
                   &model_metadata);
